...
$cname = $sales[0]['sales_custname'] ?? 'N/A';
$invoice = $sales[0]['invoice_number'] ?? $invoice_num;
$date = $sales[0]['sales_date'] ?? 'N/A';
$pt = strtolower($sales[0]['sales_payment_method'] ?? 'cash');
$sales_amount = $sales[0]['sales_amount'] ?? 0;
$cust_payment = $sales[0]['sales_cust_payment'] ?? 0;
$cust_name = $sales[0]['sales_custname'] ?? 'N/A';
?>
					...
					WHERE invoice= :invoice");
									...
									<tr>
										...
										<td colspan="2"><strong style="font-size: 12px;">
												<?php
												$sdsd = $_GET['invoice'] ?? '';
												$resultas = $db->prepare("SELECT sum(sales_order_amount) as total FROM sales_order WHERE invoice= :a");
